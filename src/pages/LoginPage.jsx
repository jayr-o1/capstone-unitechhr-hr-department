...
const LoginPage = () => {
    ...
    useEffect(() => {
        const savedUniversityData = localStorage.getItem(
                ...
                if (
                    ...
                    console.log(
                        `Login successful for employee ${
                            result.employeeData?.name || employeeId
                        } at ${result.universityName}`
                    );

                    // Update localStorage with the university data
                    saveUniversityData(
                        result.universityCode || result.universityId,
                        result.universityName
                    );
                    ...
                    console.log(
                        "HR login successful, navigating to dashboard. Role:",
                        result.role
                    );
                    ...
                    const navigationTimeout = setTimeout(() => {
                        if (loading) {
                            console.log(
                                "HR login still loading after timeout, forcing navigation"
                            );
                            toast.success(
    ...
    const validateUniversityCode = async (code) => {
        ...
        try {
            ...
            if (universitySnapshot.empty) {
                console.log(
                    "No university found with code, checking if it's a document ID:",
                    code
                );
                ...
                if (allUniversities.size > 0) {
                    ...
                    allUniversities.forEach((doc) => {
                        console.log(
                            `- ID: ${doc.id}, Name: ${doc.data().name}, Code: ${
                                doc.data().code || "No code"
                            }`
                        );
                    });
                }
                ...
                setCodeValidationError(
                    "University not found. Please check your code."
                );
                setValidatingCode(false);
                return false;
            }
                    ...
                    <div className="bg-white p-4 md:p-8 rounded-lg shadow-lg w-full max-w-md border border-black">
                        ...
                        <p className="text-center text-[#75808B] font-fredoka text-sm md:text-base text-gray-600 mb-4 md:mb-6">
                            {step === 1
                                ...
                                : `Welcome! Please sign in with your employee credentials for ${
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500 mt-1">
                                            This is a unique identifier provided
                                            by your university HR department.
                                        </p>
                                    </div>
