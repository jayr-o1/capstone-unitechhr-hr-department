...
async function authenticateEmployee(employeeId, universityId, password) {
    console.log(
        "Attempting direct employee authentication:",
        employeeId,
        "at university:",
        universityId
    );
    ...
    try {
        ...
        if (!universitySnapshot.empty) {
            ...
            if (universityDocSnapshot.exists()) {
                ...
                return {
                    success: false,
                    message:
                        "University not found. Please check the code and try again.",
                };
            }
        }
        ...
        if (employeeSnapshot.empty) {
            ...
            return {
                success: false,
                message:
                    "Employee not found. Please check your ID and try again.",
            };
        }
    ...
    try {
        ...
        if (email.includes("_SYSTEM_ADMIN_")) {
            ...
            if (match) {
                ...
                const { getSystemAdminByCredentials } = await import(
                    "./adminService"
                );
                const adminResult = await withTimeout(
                    getSystemAdminByCredentials(username, password)
                );
        ...
        if (email.includes("_EMPLOYEE_LOGIN_MARKER")) {
            ...
            const employeeLoginPattern =
                /^(.+)@(.+)_EMPLOYEE_LOGIN_MARKER\.com$/;
            ...
            if (match) {
                ...
                return await withTimeout(
                    authenticateEmployee(employeeId, universityId, password)
                );
            }
        }
        ...
        if (email.includes("@") && !email.includes("_EMPLOYEE_LOGIN_MARKER")) {
            ...
            try {
                ...
                const authMappingDoc = await withTimeout(
                    getDoc(authMappingRef)
                );
                ...
                if (authMappingDoc.exists()) {
                    ...
                    if (!universityId) {
                        ...
                        return {
                            success: false,
                            message:
                                "User record is incomplete. Please contact administrator.",
                        };
                    }
                    ...
                    const universityDoc = await withTimeout(
                        getDoc(universityRef)
                    );
                    if (universityDoc.exists()) {
                        ...
                        console.warn(
                            "University document not found for ID:",
                            universityId
                        );
                    }
                    ...
                    try {
                        if (role === "hr_head") {
                            console.log(
                                "Updating lastLogin for HR Head in university collection"
                            );
                            const hrHeadRef = doc(
                                db,
                                "universities",
                            ...
                            const hrHeadDoc = await withTimeout(
                                getDoc(hrHeadRef)
                            );
                            ...
                            if (hrHeadDoc.exists()) {
                                ...
                                console.log(
                                    "HR Head lastLogin updated successfully"
                                );
                            } else {
                                console.warn(
                                    "HR Head record not found in university collection"
                                );
                            }
                        } else if (role === "hr_personnel") {
                            console.log(
                                "Updating lastLogin for HR Personnel in university collection"
                            );
                            const hrPersonnelRef = doc(
                                db,
                                "universities",
                                universityId,
                                "hr_personnel",
                            ...
                            const hrPersonnelDoc = await withTimeout(
                                getDoc(hrPersonnelRef)
                            );

                            if (hrPersonnelDoc.exists()) {
                                await withTimeout(
                                    updateDoc(hrPersonnelRef, {
                                        lastLogin: serverTimestamp(),
                                    })
                                );
                                console.log(
                                    "HR Personnel lastLogin updated successfully"
                                );
                            } else {
                                console.warn(
                                    "HR Personnel record not found in university collection"
                                );
                            }
                        }
                    } catch (updateError) {
                    ...
                    console.error(
                        "No auth mapping found for user with UID:",
                        uid
                    );
...
export const registerUser = async (
    ...
    try {
        ...
        if (isHRHead && !universityId && userMetadata.universityName) {
            // Format the university name for ID and code (e.g., "sample-university-branch")
            const formattedName = userMetadata.universityName
                .toLowerCase()
                .trim()
            ...
            // Use the formatted name directly as the ID and code
            universityId = formattedName;
            universityCode = formattedName;

            console.log("Generated university ID and code:", universityId);
            ...
            try {
                ...
                console.error("Error creating university document:", univError);
                // Continue with registration even if university creation fails
            }
        }
        ...
        try {
            ...
            console.error("Error creating auth mapping:", authMappingError);
        ...
        if (universityId) {
            try {
                ...
                console.log(
                    "Created user document in university collection:",
                    collectionName
                );
            } catch (userDocError) {
        ...
        try {
            ...
            await signOut(auth);
            console.log(
                "User signed out after registration to require manual login"
            );
        } catch (signOutError) {
