...
export const hardDeleteJob = async (jobId) => {
    try {
        ...
        if (universityId) {
            ...
            const universityApplicantsRef = collection(
                db,
                "universities",
                universityId,
                "jobs",
            ...
            const universityApplicantsSnapshot = await getDocs(
                universityApplicantsRef
            );
            ...
            for (const applicantDoc of universityApplicantsSnapshot.docs) {
                ...
                const universityInterviewsSnapshot = await getDocs(
                    universityInterviewsRef
                );
...
export const getAllJobs = async (
    ...
    try {
        ...
        if (!includeDeleted) {
            ...
            const explicitlyNotDeletedSnapshot = await getDocs(
                explicitlyNotDeletedQuery
            );
            ...
            allJobsSnapshot.forEach((doc) => {
                const data = doc.data();
                if (
                    !explicitlyNotDeletedIds.has(doc.id) &&
                    data.isDeleted !== true
                ) {
                    jobs.push({
                        id: doc.id,
...
export const createJob = async (jobData, universityId) => {
    try {
        if (!universityId) {
            return {
                success: false,
                message: "University ID is required to create a job",
            };
        }
        ...
        const jobWithMetadata = {
            ...jobData,
            id: jobId, // Add ID to job data for notifications
            universityId,
...
export const getUniversityJobs = async (
    universityId,
    includeDeleted = false
) => {
    try {
