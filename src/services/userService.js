    ...
    try {
        ...
        if (authMappingDoc.exists()) {
            ...
            if (authMapping.universityId) {
                ...
                console.log(
                    `getUserData: Looking in '${collectionName}' collection for this user`
                );

                const universityUserRef = doc(
                    db,
                    "universities",
                    authMapping.universityId,
                    collectionName,
                    userId
                );
                ...
                if (universityUserDoc.exists()) {
                    ...
                    console.log(
                        `getUserData: University user record not found in ${collectionName}`
                    );
                    return {
                        success: false,
                ...
                console.log(
                    "getUserData: User is not associated with any university"
                );
                return {
                    success: false,
...
export const updateFCMToken = async (userId, fcmToken) => {
    try {
        ...
        if (!userId || !fcmToken) {
            console.log("updateFCMToken: Missing userId or token");
            return {
                success: false,
        ...
        // First update in users collection (main FCM token storage)
        ...
        // Get the auth mapping for additional updates
        const authMappingRef = doc(db, "authMappings", userId);
        const authMappingDoc = await getDoc(authMappingRef);
        ...
        if (authMappingDoc.exists()) {
            ...
            // Also update in appropriate role-specific collection
            if (universityId) {
                let collectionName;
                if (role === "hr_head") {
                    collectionName = "hr_head";
                ...
                if (collectionName) {
                    ...
                    await updateDoc(roleSpecificRef, {
                        fcmToken: fcmToken,
                        updatedAt: new Date().toISOString(),
                    });
                    console.log(
                        `FCM token updated in ${collectionName} collection`
                    );
                }
            }
        }
