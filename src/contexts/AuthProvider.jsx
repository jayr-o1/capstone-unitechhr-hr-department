...
const AuthProvider = ({ children }) => {
        ...
        try {
            ...
            if (userData.success) {
                ...
                if (userData.data.universityId) {
                    const universityData = await getUniversityById(
                        userData.data.universityId
                    );
    ...
    useEffect(() => {
        ...
        if (hasSavedSession) {
            console.log(
                "Session restored from localStorage, skipping Firebase auth check"
            );
            setLoading(false);
        ...
        const safetyTimeout = setTimeout(() => {
            if (loading) {
                console.log(
                    "Safety timeout triggered - resetting loading state"
                );
                setLoading(false);
            }
        ...
        const unsubscribe = onAuthStateChanged(auth, async (authUser) => {
            ...
            try {
                if (authUser) {
                    console.log(
                        "Setting user from Firebase Auth:",
                        authUser.uid
                    );
                    setUser(authUser);
                    await fetchUserDetails(authUser.uid);
                    console.log(
                        "User details fetched, setting loading to false"
                    );
                    setLoading(false);
                } else {
                    console.log(
                        "No Firebase Auth user, clearing state and setting loading to false"
                    );
                    setUser(null);
                    setUserDetails(null);
                    setUniversity(null);
        ...
        if (loginAttemptTimestamp && loading) {
            const timeoutId = setTimeout(() => {
                ...
                if (timeElapsed > 8000 && loading) {
                    console.log(
                        "Login has been in loading state for too long, forcing reset"
                    );
                    setLoading(false);
                }
            }, 8000);
        ...
        try {
            ...
            if (result.role === "system_admin") {
                ...
                const adminUser = {
                    ...
                    displayName:
                        result.adminUser?.displayName || "System Administrator",
                    email: `${
                        result.adminUser?.username || "admin"
                    }@system.admin`,
                };
                ...
                console.log(
                    "System admin login successful - user object and details set and saved to localStorage"
                );
                setLoading(false);
            }
            // For employee logins, we need to set up user details differently
                ...
                if (!auth.currentUser) {
                    ...
                    const employeeUser = {
                        uid: `emp_${result.employeeData?.id || ""}_${
                            result.universityId
                        }`,
                        isEmployee: true,
                    ...
                    console.log(
                        "Employee login successful - user object and details set and saved to localStorage"
                    );
                    setLoading(false);
                } else {
