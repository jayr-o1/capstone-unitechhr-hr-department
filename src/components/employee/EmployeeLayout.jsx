          ...
          >
            ...
            <nav className="flex-1 overflow-y-auto py-4 px-3 md:mt-0 mt-4">
              <ul className="space-y-3">
                {navigation.map((item) => (
                  <li key={item.name} className="bg-gray-100 rounded-xl shadow-md hover:bg-gray-200 transition-colors duration-200">
                    <Link
                      to={item.path}
                      className={`flex items-center py-3 px-4 ${
                        location.pathname === item.path ? 'bg-gray-200 rounded-xl text-blue-600' : 'text-gray-600'
                      }`}
